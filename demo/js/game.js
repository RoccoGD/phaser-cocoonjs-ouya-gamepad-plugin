...
function create() {
  ...
  ouyaGamepad = game.plugins.add(Phaser.Plugin.CocoonJSOuyaGamepad);
  ...
  pad1 = ouyaGamepad.gamepads.pad1;
  pad2 = ouyaGamepad.gamepads.pad2;
  pad3 = ouyaGamepad.gamepads.pad3;
  pad4 = ouyaGamepad.gamepads.pad4;
...
function onStartInput() {
  if (ouyaGamepad.gamepads.active) {
    ouyaGamepad.stopPolling();
    startInputButton.setFrames(0, 0, 0);
  } else {
    ouyaGamepad.startPolling();
    startInputButton.setFrames(1, 1, 1);
  }
}

function onResetButtons() {
  ouyaGamepad.gamepads.reset();
}

function update() {
  supportedText.setText('Gamepad supported in this browser: ' + ouyaGamepad.gamepads.supported);
  activeText.setText('Gamepad input active: ' + ouyaGamepad.gamepads.active);
  gamepadCountText.setText('Gamepads connected: ' + ouyaGamepad.gamepads.padsConnected);
...
function setupText() {
  ...
  var tinyStyle = {font: "12px Arial", fill: "#ffffff", align: "left"};

  supportedText = game.add.text(10, 10, 'Gamepad supported in this browser: ' + ouyaGamepad.gamepads.supported, style);
  activeText = game.add.text(10, 30, 'Gamepad input active: ' + ouyaGamepad.gamepads.active, style);
  gamepadCountText = game.add.text(10, 50, 'Gamepads connected: ' + ouyaGamepad.gamepads.padsConnected, style);
  ...
  ouyaGamepad.gamepads.addCallbacks(this, {
